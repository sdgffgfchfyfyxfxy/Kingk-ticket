...
from database.db import (
    ...
    get_ticket_messages, get_ticket_replies, close_ticket
)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_sticker("CAACAgIAAxkBAAE..." ) # استیکر خوشامدگویی نمونه
...
async def receive_ticket_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎁 داش، حالا متن، عکس، ویدیو یا فایل تیکتت رو بفرست همینجا ✨️\n\n(حداکثر ۳ پیام. پس از ارسال هر پیام، دکمه ✅ تایید را در پایین لمس کنید)")
    
    from telegram import InlineKeyboardButton, InlineKeyboardMarkup
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ تایید", callback_data="show_confirm")]])
    await update.message.reply_text("لطفاً پیام‌های خود را ارسال کنید و در نهایت دکمه زیر را بزنید:", reply_markup=kb)
...
async def receive_ticket_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ربات بعد از هر پیام هیچ متنی ارسال نکند (به جز ذخیره تعداد)
    return TicketStates.CONTENT
